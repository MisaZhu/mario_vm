...
void lex_get_nextch(lex_t* lex) {
	...
	if (lex->dataPos < lex->dataEnd){
		lex->nextCh = lex->data.cstr[lex->dataPos];
	}else{
		lex->nextCh = 0;
	}
	lex->dataPos++;
...
void lex_init(lex_t * lex, const char* input) {
	str_init(&lex->data);
	str_cpy(&lex->data, input);
	lex->dataStart = 0;
	lex->dataEnd = lex->data.len;
	lex_reset(lex);
}

void lex_free(lex_t* lex) {
	str_release(&lex->data);
...
char* bc_getstr(bytecode_t* bc, int i) {
	if(i<0 || i == 0xFFFF ||  i>=bc->strTable.size)
		return "";
	return (char*)bc->strTable.items[i];
}	
